...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        br.lines()
            ...
            .map(line -> line.split(","))
            .map(cols -> new GooglePlayStoreData(cols[0].trim(), cols[1].trim(), Float.parseFloat(cols[2].trim())))
        ...
        for (String c: data.keySet()) {
            ...
            System.out.printf("Category: %s\nHighest: %s, %f\nLowest: %s, %f\nAverage: %f\nDiscarded: %d\n\n", c,
                    s.getHighestRatedApp(), s.getHighestRating(), s.getLowestRatedApp(), s.getLowestRating(), s.avgRating(), discard);
